...
public final class TopoSort {
    public static int[] kahn(Graph dag, Metrics m){
        ...
        for(int u=0;u<n;u++) for(var e: dag.adj.get(u)){ indeg[e.to()]++; m.tick("topo.edges"); }
        ...
        while(!q.isEmpty()){
            ...
            for(var e: dag.adj.get(u)){ if(--indeg[e.to()]==0) q.add(e.to()); m.tick("topo.reductions"); }
        }
        if(idx!=n) throw new IllegalStateException("Graph not DAG");
        return order;
    }
}
